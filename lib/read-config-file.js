...
const readConfigFile = (CZ_CONFIG_NAME = '.cz-config.js') => {
  ...
  if (pkg) {
    ...
    if (pkg.config && pkg.config['cz-custom'] && pkg.config['cz-custom'].config) {
      // resolve relative to discovered package.json
      const pkgPath = path.resolve(pkgDir, pkg.config['cz-custom'].config);

      log.info('>>> Using cz-custom config specified in your package.json: ', pkgPath);
  ...
  log.error(
    'Unable to find a configuration file. Please refer to documentation to learn how to set up: https://github.com/XPoet/cz-customizable#configuration "',
  );
  return null;
};
